...
namespace CodeCraftNet.Infrastructure.Seeding;

public static class CodeCraftNetSeeder
{
    public static async Task SeedAsync(IServiceProvider serviceProvider, CancellationToken cancellationToken = default)
    {
        using var scope = serviceProvider.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<CodeCraftNetDbContext>();
    ...
    private static async Task SeedTopicsAsync(CodeCraftNetDbContext dbContext, DateTime now, CancellationToken cancellationToken)
    {
    ...
    private static async Task TopUpChallengePoolAsync(CodeCraftNetDbContext dbContext, DateTime now, CancellationToken cancellationToken)
    {
    ...
    private static async Task TopUpQuestionPoolAsync(CodeCraftNetDbContext dbContext, DateTime now, CancellationToken cancellationToken)
    {
        var topicIdsBySlug = await dbContext.Topics
            .ToDictionaryAsync(topic => topic.Slug, topic => topic.Id, cancellationToken);
    ...
    private static IEnumerable<QuestionSpec> QuestionPool()
    {
        ...
        yield return Mc(csharp, TopicDifficulty.Fundamental,
            ...
            60, ["types"], ("Sınıf (class Order)", false), ("Değer Yapısı (record struct Money)", true), ("Arayüz (interface IOrder)", false), ("Temsilci (delegate void Handler)", false));
        ...
        yield return Mc(csharp, TopicDifficulty.Intermediate,
            "C#'ta 'record' ve 'class' arasındaki temel eşitlik farkı nedir?",
            "Record türleri varsayılan olarak değere dayalı (value-based) eşitlik sağlarken, class türleri referans eşitliği kullanır.",
            60, ["records"], ("Record'lar referans eşitliği, class'lar değer eşitliği kullanır", false), ("Record'lar varsayılan olarak değere dayalı eşitlik sağlar", true), ("Record ve class eşitlik açısından tamamen aynıdır", false));
        yield return Sa(csharp, TopicDifficulty.Advanced,
            "Heap üzerinde ek bellek tahsisi yapmadan (zero-allocation) yüksek performanslı bellek dilimleme işlemleri sağlayan C# türü nedir?",
            "Span veya ReadOnlySpan bellek üzerinde kopyalama yapmadan performanslı dilimleme sağlar.",
            45, ["performance", "memory"], "span", "readonlyspan", "span<t>");
        yield return Mc(csharp, TopicDifficulty.Fundamental,
            "C#'ta bir metodun içerisinde 'yield return' kullanıldığında dönen koleksiyon ne zaman oluşturulur?",
            "Koleksiyon elemanları üzerinde iterasyon yapıldıkça (numaralandırıldıkça) tembelce (lazy) oluşturulur.",
            60, ["yield", "linq"], ("Metot çağrıldığı anda tüm elemanlar bellekte oluşturulur", false), ("İterasyon yapıldıkça elemanlar ihtiyaç anında üretilir", true), ("İlk eleman alındıktan sonra metot sonlanır", false));
        yield return Sa(csharp, TopicDifficulty.Intermediate,
            "Bir hatayı fırlatırken (exception handling) mevcut çağrı yığınını (stack trace) sıfırlamadan yeniden fırlatmak için hangi deyim kullanılır?",
            "throw; deyimi çağrı yığınını koruyarak hatayı üst katmana iletir.",
            30, ["exceptions"], "throw;", "throw");
        yield return Mc(csharp, TopicDifficulty.Intermediate,
            "C#'ta switch ifadesinde veya pattern matching işleminde '_' (alt çizgi) sembolü neyi temsil eder?",
            "Hiçbir kalıba uymayan tüm diğer durumları (default / discard) temsil eder.",
            45, ["pattern-matching"], ("Null değerleri", false), ("Tüm diğer eşleşmeyen durumları (discard)", true), ("Sadece sayısal değerleri", false));
        ...
        yield return Mc(aspnet, TopicDifficulty.Intermediate,
            "HTTP PUT ve PATCH metotları arasındaki temel anlamsal fark nedir?",
            "PUT hedef kaynağı tamamen değiştirirken, PATCH kaynağın belirli alanlarını kısmi olarak günceller.",
            60, ["http"], ("PUT kısmi güncelleme yapar, PATCH tüm kaynağı değiştirir", false), ("PUT kaynağı tamamen değiştirir, PATCH sadece belirtilen alanları kısmi günceller", true), ("İkisi arasında hiçbir fark yoktur", false));
        yield return Sa(aspnet, TopicDifficulty.Fundamental,
            "ASP.NET Core Bağımlılık Enjeksiyonunda (DI), her HTTP isteği (request) için tek bir örnek oluşturan ömür seçeneği nedir?",
            "Scoped (AddScoped) her HTTP isteği boyunca aynı servisi sunar.",
            30, ["di"], "scoped", "addscoped");
        yield return Mc(aspnet, TopicDifficulty.Fundamental,
            "İstemcinin bir kaynağa erişim yetkisinin bulunmadığını gösteren standart HTTP durum kodu nedir?",
            "403 Forbidden yetkisiz erişimi ifade eder (401 ise kimlik doğrulamasız kalındığını gösterir).",
            45, ["http"], ("401 Unauthorized", false), ("403 Forbidden", true), ("404 Not Found", false));
        ...
        yield return Sa(ef, TopicDifficulty.Intermediate,
            "EF Core'da yapılan model değişikliklerini veritabanı şemasına yansıtmak üzere oluşturulan kod bazlı sürümlere ne ad verilir?",
            "Migration'lar veritabanı şemasını zaman içinde sürümler ve günceller.",
            30, ["migrations"], "migration", "migrations");
        yield return Mc(ef, TopicDifficulty.Intermediate,
            "DbContext ömrü (service lifetime) varsayılan olarak bağımlılık enjeksiyonunda nasıl kaydedilmelidir?",
            "DbContext her HTTP isteği (request) için ayrı bir örnek olacak şekilde Scoped kaydedilmelidir.",
            45, ["di", "ef-core"], ("Singleton", false), ("Scoped", true), ("Transient", false));
        ...
        yield return Mc(clean, TopicDifficulty.Fundamental,
            "Temiz Mimari'de Domain (Alan) katmanının dış kütüphanelere ve veritabanı paketlerine bağımlılığı nasıl olmalıdır?",
            "Domain katmanı tamamen bağımsız olmalı, sıfır dış kütüphane bağımlılığı taşımalıdır.",
            60, ["domain"], ("ORM paketlerine doğrudan bağımlı olmalıdır", false), ("Tamamen saf olmalı, dış kütüphanelere bağımlı olmamalıdır", true), ("Sadece UI paketlerine bağımlı olmalıdır", false));
        yield return Sa(clean, TopicDifficulty.Advanced,
            "Domain odaklı tasarımda (DDD), bir grup ilişkili varlığın tutarlılık sınırını (consistency boundary) yöneten ana nesneye ne ad verilir?",
            "Aggregate Root (Bütüncül Kök) grup içerisindeki değişimlerin tutarlılığını garanti eder.",
            45, ["ddd"], "aggregate root", "aggregate", "bütüncül kök");
        ...
        yield return Sa(cqrs, TopicDifficulty.Intermediate,
            ".NET ekosisteminde CQRS komut ve sorgularını ilgili handler sınıflarına yönlendirmek için en sık kullanılan kütüphane hangisidir?",
            "MediatR kütüphanesi içiçe geçmiş boru hattı (pipeline) davranışları ile popülerdir.",
            30, ["mediatr"], "mediatr");
        ...
        yield return Mc(jwt, TopicDifficulty.Fundamental,
            ...
            "header.payload.signature — base64url ile kodlanmış iki JSON bölümü artı mevcudiyet doğrulaması sağlayan bir imza.",
        ...
        yield return Mc(jwt, TopicDifficulty.Intermediate,
            "JWT token içinde kullanıcı rollerini ve kimlik bilgilerini taşıyan key-value anahtar çiftlerine ne ad verilir?",
            "Claim adı verilen yapılar kullanıcı özniteliklerini ve yetki rollerini taşır.",
            45, ["claims"], ("Header", false), ("Claim", true), ("Scope", false));
        ...
        yield return Sa(caching, TopicDifficulty.Intermediate,
            "Önbellekteki verinin her erişimde süresinin yenilenmesini sağlayan esnek süre mekanizmasının adı nedir?",
            "Kayan süre (Sliding Expiration) veriye erişildikçe ömrünü uzatır.",
            45, ["expiration"], "kayan süre", "sliding expiration");
        ...
        yield return Sa(pg, TopicDifficulty.Intermediate,
            "PostgreSQL'de JSON verilerini indekslenebilir ve verimli ikili (binary) biçimde saklayan veri tipi hangisidir?",
            "jsonb verileri sıkıştırılmış ikili formatta tutarak hızlı arama ve indeksleme sağlar.",
            30, ["jsonb"], "jsonb");
    }
}
