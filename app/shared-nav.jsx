...
function TopNav({ activeTab, apiOnline }) {
  ...
  const tabs = [
    { id: 'today',    label: 'Today',    href: 'Dashboard.html' },
  ...
  const dotColor = apiOnline === false ? 'var(--danger)'
                ...
                : 'var(--accent)';
  const dotLabel = apiOnline === false ? 'API offline'
                : apiOnline === 'demo' ? 'Demo'
                : 'Live';
      ...
      <div className="container-x flex items-center justify-between" style={{ height: 60 }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 pl-3"
            ...
            <button className="btn-icon ml-1" onClick={onSignOut} title="Sign out" aria-label="Sign out">
              <Icon.Caret />
            </button>
          </div>
        </div>
